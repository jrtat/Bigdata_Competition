...
获取沪深300指数成分股历史数据
- 获取当前沪深300成分股列表
- 抓取每只股票在指定时间窗口内（默认2024-01-01至今）的历史日线数据
- 数据字段：股票代码,日期,开盘,收盘,最高,最低,成交量,成交额,振幅,涨跌额,换手率,涨跌幅,
           市盈率,市销率,市现率,市净率,交易状态,是否ST
- 使用baostock平台，后复权数据
- 增量模式：自动检测已有数据，仅更新缺失或新出现的时间段，保持同一股票数据相邻存储
- 输出文件：./data/stock_data.csv，同时保存成分股列表和失败股票列表
"""
...
def get_stock_history(bs_code, start_date, end_date):
    """获取单只股票历史数据（扩充版，已去除复权状态列）"""
    # 扩充字段，包含除adjustflag外的所有日线可用指标
    rs = bs.query_history_k_data_plus(bs_code,
                                      ...
                                      frequency="d", adjustflag="1")  # adjustflag="1"表示后复权，无需在数据中返回
...
def merge_stock_data(existing_df, new_df, stock_code):
    ...
    # 获取该股票的现有数据
    stock_existing = existing_df[existing_df['股票代码_str'] == stock_code].drop(
        columns=['股票代码_str']) if stock_code in existing_df['股票代码_str'].values else pd.DataFrame()
        ...
        for idx, row in hs300_df.iterrows():
            ...
            if existing_min_date and existing_max_date:
                ...
                if need_early:
                    ...
                    fetch_end = (datetime.strptime(existing_min_date, '%Y-%m-%d') - pd.Timedelta(days=1)).strftime(
                        '%Y-%m-%d')
